    ...
    <form>
      ...
      <div className='w-full flex max-sm:flex-col sm:gap-4 mt-4'>
             <div className='flex-1 max-w-48'>
                     ...
                     <select value={inputs.roomType} className='border opacity-70 border-gary-300/60 mt-1 rounded p-2 w-full outline-none' onChange={(e)=>{seInputs({...inputs,roomType:e.target.value})}}>
             ...
             <div>
              ...
              <input type="number" name="" id="" placeholder='0' className='border border-gray-300 mt-1 rounded p-2 w-24 outline-none' value={inputs.pricePerNight} onChange={ e=>seInputs({...inputs,pricePerNight:e.target.value})}/>
             </div>
      </div>
      ...
      <div className='flex flex-col flex-wrap mt-1 text-gray-400 max-w-sm'>
           {Object.keys(inputs.aminities).map((key,i)=>(
            <div key={i}>
              <input type="checkbox" name="" id={`amenities${i+1}`} checked={inputs.aminities[key]} onChange={e=>seInputs({...inputs,aminities:{...inputs.aminities, [key]:!inputs.aminities[key]}})} className='bg-transparent'/>
